...
class Metric(ABC):
    ...
    def __call__(self, value, weight=1.0):
...
class MaxMetric(Metric):
    ...
    def update(self, value, weight=1.0):
        value = value.detach()
        max_val = torch.max(value) if value.numel() > 1 else value
        self.max_value = torch.maximum(self.max_value.to(max_val.device), max_val)
...
class MinMetric(Metric):
    ...
    def update(self, value, weight=1.0):
        value = value.detach()
        min_val = torch.min(value) if value.numel() > 1 else value
        self.min_value = torch.minimum(self.min_value.to(min_val.device), min_val)
...
def nll_chi_loss(
    ...
    if chi_num is not None:
        assert 1 <= chi_num and chi_num <= 4, "Invalid chi_num supplied to NLL loss fn"
        chi_idx = chi_num - 1
        loss = loss[:, chi_idx]
...
def offset_mse(
    ...
    # Mean per chi
    if chi_num is not None:
        assert 1 <= chi_num and chi_num <= 4, "Invalid chi_num supplied to offset MSE loss fn"
        chi_idx = chi_num - 1
        offset_pred = offset_pred[:, chi_idx]
        offset_true = offset_true[:, chi_idx]
        mask = mask[:, chi_idx]
    
    # Computes loss
    if avg_bb_plddt is None:
        avg_bb_plddt = torch.ones(offset_pred.shape[0],
                                  dtype=offset_pred.dtype,
...
def sc_rmsd(
    ...
    if per_residue_first:
        ...
        if avg_bb_plddt is None:
            avg_bb_plddt = torch.ones(
                rmsd.shape[0],
                dtype=rmsd.dtype,
                device=rmsd.device,
            )
        weighted_rmsd = avg_bb_plddt * rmsd

        # Aggregates RMSD values across residues
        # if _metric is not None and not isinstance(_metric, BlackHole):
        #     mse = _metric(rmsd)
        # else:
...
def local_interresidue_sc_clash_loss(
    ...
    residx_atom14_to_atom37 = restype_atom14_to_atom37[aatype]
    atom14_atom_exists = batch.atom14_mask.clone()
    residue_index = (1 + torch.arange(batch.pos_CA.shape[0], device=batch.pos_CA.device)).long()
    ...
    # shape (N_pairs, 14, 3)
    res1_atom14_xyz = atom14_pred_positions.clone()[valid_pairs[0]]
    res2_atom14_xyz = atom14_pred_positions.clone()[valid_pairs[1]]
...
def unclosed_proline_loss(batch, atom14_pred_positions, tolerance_factor=12, _metric=None, eps=1e-10) -> torch.Tensor:
    ...
    pro_mask = (batch.seq == rc.restype_order['P']).float()
    # print(f'pro_mask = {pro_mask}')
